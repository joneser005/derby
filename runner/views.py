...
def getSwapCandidates(request, race_id):
    ...
    if isNotControl(request):
        log.warning("Unauthorized call to getSwapCandidates from host [{}].".format(request.get_host()))
        return HttpResponse('403: Failed')
...
def swapRacers(request, race_id):
    ...
    if isNotControl(request):
        log.warning("Unauthorized call to swapRacers from host [{}].".format(request.get_host()))
        return HttpResponse('403: Failed')
    ...
    lane = args['lane']
    log.debug('race_id={}, run_seq_1={}, racer_id_1={}, run_seq_2={}, racer_id_2={}, lane={}'.format(race_id, run_seq_1,
                                                                                                     racer_id_1,
                                                                                                     run_seq_2,
                                                                                                     racer_id_2, lane))
...
def setRunResult(request, race_id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        log.warning("Wrong request type from host [{}].".format(request.get_host()))
